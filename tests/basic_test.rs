...
fn test_nested_condition() {
    ...
    match engine.render(String::from("{% if myvar - 2 %}Hidden{% else %}TEST{% if 1 %}!!!{% endif %} Displayed{{ myvar }}{% endif %}"), variables) {
        Ok(string) => assert_eq!(string, String::from("TEST!!! Displayed2")),
        Err(e) => panic!("Error: {}", &e.message)
    }
}